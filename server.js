...
class GameState {
    ...
    tick() {
        ...
        // Broadcast every tick for smooth updates
        this.broadcastGameState();
    }
    ...
    handlePlayerInput(playerId, input) {
        ...
        // Movement
        if (input.movement) {
            const moveX = input.movement.x * player.speed;
            const moveY = input.movement.y * player.speed;
            player.x += moveX;
            player.y += moveY;
        ...
        // Abilities (Q, E, R)
        if (input.ability) {
            this.handleAbility(player, input.ability);
        }
    }

    handleAbility(player, ability) {
        const hero = HEROES.find(h => h.id === player.heroId);
        if (!hero) return;

        // Simple ability effects - can be expanded
        switch (ability) {
            case 'q':
                // First ability - dash in direction
                player.x += player.direction.x * 150;
                player.y += player.direction.y * 150;
                player.x = Math.max(20, Math.min(this.map.width - 20, player.x));
                player.y = Math.max(20, Math.min(this.map.height - 20, player.y));
                break;
            case 'e':
                // Second ability - area damage
                    ...
                    if (p.id !== player.id) {
                        ...
                        if (dist < 200) {
                            ...
                            if (p.health <= 0) {
                                player.kills++;
                                p.deaths++;
                                this.killScores.set(player.id, (this.killScores.get(player.id) || 0) + 1);
    ...
    respawnPlayer(player) {
        setTimeout(() => {
            const spawn = this.map.spawnPoints[Math.floor(Math.random() * this.map.spawnPoints.length)];
            player.x = spawn.x;
            player.y = spawn.y;
