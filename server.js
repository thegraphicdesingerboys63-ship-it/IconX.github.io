...
class GameState {
    ...
    generateMap() {
        ...
        return { width, height, walls, obstacles, spawnPoints };
    }
...
function handleMessage(playerId, ws, data) {
    switch (data.type) {
        case 'guestLogin':
            const username = data.username || `Guest_${playerId.slice(0, 6)}`;
            const friendCode = generateFriendCode();
            players.set(playerId, { ws, username, status: 'online', currentGame: null, heroId: 'blaze', friendCode });
            accounts.set(friendCode, { username, playerData: { currency: 500, ownedSkins: {}, equippedSkins: {}, stats: { totalKills: 0, totalDeaths: 0, matchesPlayed: 0, wins: 0 }, friends: [] } });
            ws.send(JSON.stringify({ type: 'loginResult', success: true, playerId, friendCode, playerData: accounts.get(friendCode).playerData }));
            break;
